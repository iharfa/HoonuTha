...
const seedRows = () =>
  buildSeed().map((r, i) => ({
    ...
    ambient: r.airTemp ?? null, // seed: the day's ambient == the sample air temp
    place: r.place ?? "",
    by: r.by ?? "",
    grp: "demo",
    at: r.at,
...
async function pg() {
  ...
  if (!ensured) {
    ...
    // Migrate tables created before shade_source / grp / ambient existed.
    ...
    await sql`ALTER TABLE readings ADD COLUMN IF NOT EXISTS ambient real`;
...
async function seedSql(sql) {
  const rows = seedRows().map((r) => ({
    ...
    temp: r.temp, air_temp: r.airTemp, ambient: r.ambient, place: r.place, by_name: r.by, at_ms: r.at,
  }));
  await sql`INSERT INTO readings (material, color, shade, shade_source, temp, air_temp, ambient, place, by_name, grp, at)
    SELECT material, color, shade, shade_source, temp, air_temp, ambient, place, by_name, 'demo', to_timestamp(at_ms / 1000.0)
    FROM jsonb_to_recordset(${JSON.stringify(rows)}::jsonb)
    AS x(material text, color text, shade text, shade_source text, temp real, air_temp real, ambient real, place text, by_name text, at_ms bigint)`;
}
const fromPg = (row) => ({
  id: row.id, material: row.material, color: row.color, shade: row.shade,
  shadeSource: row.shade_source, temp: row.temp, airTemp: row.air_temp, ambient: row.ambient ?? null,
...
// Reject junk values at the trust boundary; null is fine (e.g. GPS denied).
const coord = (v, max) => (Number.isFinite(Number(v)) && Math.abs(Number(v)) <= max && v !== null && v !== "" ? Number(v) : null);
const inRange = (v, lo, hi) => (Number.isFinite(Number(v)) && Number(v) >= lo && Number(v) <= hi ? Number(v) : null);
// Trust the client's measurement time (so offline readings keep their real
// time), but clamp out clock skew / spoofing: no future, no older than ~1 year.
const stamp = (v) => {
  const now = Date.now(), t = Number(v);
  return Number.isFinite(t) && t > now - 370 * 864e5 && t < now + 864e5 ? t : now;
};
...
export async function addReading(r, grp = "demo") {
  const rec = {
    ...
    shadeSource: r.shadeSource ?? null,
    temp: Number(r.temp),
    airTemp: r.airTemp != null && r.airTemp !== "" ? Number(r.airTemp) : null,
    ambient: inRange(r.ambient, -10, 60), // the day's Maldives air temp, captured automatically
    place: r.place || "", by: r.by || "", lat: coord(r.lat, 90), lon: coord(r.lon, 180), grp,
    at: stamp(r.at),
  };
  if (!url()) {
    const db = fileDb();
    const row = { id: db.nextId++, ...rec };
    db.readings.push(row);
    saveFile(db);
    return row;
  }
  const sql = await pg();
  const [row] = await sql`INSERT INTO readings (material, color, shade, shade_source, temp, air_temp, ambient, place, by_name, lat, lon, grp, at)
    VALUES (${rec.material}, ${rec.color}, ${rec.shade}, ${rec.shadeSource}, ${rec.temp}, ${rec.airTemp}, ${rec.ambient}, ${rec.place}, ${rec.by}, ${rec.lat}, ${rec.lon}, ${grp}, ${new Date(rec.at).toISOString()})
    RETURNING *`;
  return fromPg(row);
}
