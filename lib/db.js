...
async function pg() {
  ...
  if (!ensured) {
    ...
    // Migrate tables created before shade_source existed.
...
async function seedSql(sql) {
  ...
  await sql`INSERT INTO readings (material, color, shade, shade_source, temp, air_temp, place, by_name, at)
    SELECT material, color, shade, shade_source, temp, air_temp, place, by_name, to_timestamp(at_ms / 1000.0)
...
export async function addReading(r) {
  ...
  const sql = await pg();
  const [row] = await sql`INSERT INTO readings (material, color, shade, shade_source, temp, air_temp, place, by_name)
    VALUES (${rec.material}, ${rec.color}, ${rec.shade}, ${rec.shadeSource}, ${rec.temp}, ${rec.airTemp}, ${rec.place}, ${rec.by})
    RETURNING *`;
  return fromPg(row);
}
